...
import type { RiskCreate, RiskCategory, ThreatSource, LikelihoodLevel, ImpactLevel, RiskTreatment, Asset } from '~/types/asset-risk'
...
const handleSubmit = async () => {
  if (!formData.value.title.trim() || !formData.value.risk_owner.trim() || formData.value.asset_ids.length === 0) {
  ...
  try {
    await riskApi.createRisk(props.organizationId, {
      ...formData.value,
    ...
    error.value = err.message || 'Failed to report risk'
  } finally {
          ...
          >
            ...
            <div class="flex items-center justify-between p-6 sm:p-8 border-b border-gray-100">
              <div>
                ...
                <p class="text-[14px] text-[#4f9690] mt-1">Identify and document a new security risk.</p>
              </div>
              <button
                @click="handleClose"
                :disabled="isLoading"
            ...
            <form @submit.prevent="handleSubmit" class="flex-1 overflow-y-auto p-6 sm:p-8 space-y-8">
              ...
              <div class="space-y-6">
                ...
                <div class="space-y-3">
                  ...
                  <p v-if="assets.length === 0" class="text-[12px] text-[#4f9690] italic">No assets available. Please add assets first.</p>
                </div>
            ...
            <div class="p-6 sm:p-8 border-t border-gray-100 bg-gray-50/50 flex flex-col-reverse sm:flex-row items-center justify-end gap-3">
              ...
              >
                <span v-if="isLoading">Reporting...</span>
                <span v-else>Report Risk</span>
                <div
                  v-if="isLoading"
