      ...
      <div v-else-if="org" class="space-y-8">
        ...
        <header class="relative z-10">
          <nav class="mb-3 sm:mb-4">
            <ol class="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm text-gray-500 overflow-x-auto">
              ...
              <li class="font-medium text-gray-900 truncate">{{ org.name }}</li>
            </ol>
          </nav>

          <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div class="flex-1 min-w-0">
              <div class="flex items-start sm:items-center gap-3 sm:gap-4 mb-2">
                <div class="w-12 h-12 sm:w-16 sm:h-16 rounded-xl bg-gradient-to-br from-[#09423C] to-[#1a6b61] flex items-center justify-center text-white text-xl sm:text-2xl font-bold shadow-lg shadow-[#09423C]/20 flex-shrink-0">
                  {{ org.name.substring(0, 1).toUpperCase() }}
                </div>
                <div class="flex-1 min-w-0">
                  <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 tracking-tight truncate">
                    {{ org.name }}
                  </h1>
                  <div class="flex flex-wrap items-center gap-1.5 sm:gap-2 mt-1">
                    <span v-if="org.industry" class="text-xs sm:text-sm text-gray-500 bg-white border border-gray-200 px-2 py-0.5 rounded-md shadow-sm whitespace-nowrap">
                      {{ org.industry }}
                    </span>
                    <span v-if="org.is_active" class="flex items-center gap-1.5 px-2 sm:px-2.5 py-0.5 bg-green-50 border border-green-200/50 text-green-700 text-xs font-medium rounded-full ring-1 ring-inset ring-green-600/20 whitespace-nowrap">
                      <span class="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></span>
                      Active
                    </span>
                    <span v-else class="flex items-center gap-1.5 px-2 sm:px-2.5 py-0.5 bg-red-50 border border-red-200/50 text-red-700 text-xs font-medium rounded-full ring-1 ring-inset ring-red-600/20 whitespace-nowrap">
                      Inactive
                    </span>
                  </div>
                </div>
              </div>
              <p v-if="org.description" class="text-sm sm:text-base text-gray-500 max-w-2xl mt-2 leading-relaxed">
                {{ org.description }}
              </p>
            </div>
        ...
        <div class="border-b border-gray-200 overflow-x-auto -mx-4 sm:mx-0 px-4 sm:px-0">
          <nav class="flex space-x-4 sm:space-x-8 min-w-max sm:min-w-0" aria-label="Tabs">
            <button
              v-for="tab in [
                { id: 'overview', label: 'Overview', shortLabel: 'Overview', icon: 'M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z' },
                { id: 'members', label: 'Team Members', shortLabel: 'Members', icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z' },
                { id: 'invitations', label: 'Invitations', shortLabel: 'Invites', icon: 'M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z' },
                { id: 'roles', label: 'Roles & Permissions', shortLabel: 'Roles', icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' }
              ]"
              :key="tab.id"
              @click="activeTab = tab.id as any"
              :class="[
                'group inline-flex items-center py-3 sm:py-4 px-1 border-b-2 font-medium text-xs sm:text-sm transition-all duration-200 gap-1.5 sm:gap-2 whitespace-nowrap flex-shrink-0',
                activeTab === tab.id
                  ? 'border-[#09423C] text-[#09423C]'
                  : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
              ]"
            >
              <svg class="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0" :class="activeTab === tab.id ? 'text-[#09423C]' : 'text-gray-400 group-hover:text-gray-500'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="tab.icon" />
              </svg>
              <span class="hidden sm:inline">{{ tab.label }}</span>
              <span class="sm:hidden">{{ tab.shortLabel }}</span>
              <span v-if="tab.id === 'members'" class="bg-gray-100 text-gray-600 ml-1 py-0.5 px-1.5 sm:px-2 rounded-full text-xs flex-shrink-0">{{ members.length }}</span>
              <span v-if="tab.id === 'invitations' && invitations.length > 0" class="bg-red-100 text-red-600 ml-1 py-0.5 px-1.5 sm:px-2 rounded-full text-xs flex-shrink-0">{{ invitations.length }}</span>
            </button>
          </nav>
        </div>
        ...
        <main>
          ...
          >
            <div v-if="activeTab === 'overview'" class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
              ...
              <!-- Security Assessment Section - Mobile: At Top, Desktop: Right Sidebar -->
              <div class="lg:col-span-1 order-1 lg:order-3">
                 <div class="relative overflow-hidden rounded-xl sm:rounded-2xl bg-[#09423C] text-white p-4 sm:p-6 shadow-xl h-full flex flex-col justify-between">
                    <div class="absolute -top-10 -right-10 w-40 h-40 bg-white/10 rounded-full blur-2xl"></div>
                    <div class="absolute bottom-0 right-0 w-32 h-32 bg-teal-400/20 rounded-full blur-xl"></div>
                    
                    <div class="relative z-10">
                       <div class="w-10 h-10 sm:w-12 sm:h-12 bg-white/20 backdrop-blur-sm rounded-lg sm:rounded-xl flex items-center justify-center mb-3 sm:mb-4">
                          <svg class="w-5 h-5 sm:w-6 sm:h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
                       </div>
                       <h3 class="text-lg sm:text-xl font-bold mb-2">Security Assessment</h3>
                       <p class="text-teal-100 text-xs sm:text-sm mb-4 sm:mb-6 leading-relaxed">
              ...
              <!-- Stats Cards -->
              <div class="lg:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 order-2 lg:order-1">
                <div class="bg-white p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                  <div class="flex items-center gap-3 sm:gap-4">
                    <div class="p-2 sm:p-3 bg-blue-50 text-blue-600 rounded-lg sm:rounded-xl flex-shrink-0">
                    ...
                    <div class="min-w-0">
                      <p class="text-xs sm:text-sm font-medium text-gray-500">Total Members</p>
                ...
                <div class="bg-white p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                  <div class="flex items-center gap-3 sm:gap-4">
                     <div class="p-2 sm:p-3 bg-purple-50 text-purple-600 rounded-lg sm:rounded-xl flex-shrink-0">
                    ...
                    <div class="min-w-0">
                      <p class="text-xs sm:text-sm font-medium text-gray-500">Pending Invites</p>
                      <p class="text-xl sm:text-2xl font-bold text-gray-900">{{ invitations.length }}</p>
                    </div>
                  </div>
                </div>
                <div class="bg-white p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                  <div class="flex items-center gap-3 sm:gap-4">
                    <div class="p-2 sm:p-3 bg-teal-50 text-teal-600 rounded-lg sm:rounded-xl flex-shrink-0">
                      <svg class="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
                    </div>
                    <div class="min-w-0">
                      <p class="text-xs sm:text-sm font-medium text-gray-500">Active Roles</p>
                      <p class="text-xl sm:text-2xl font-bold text-gray-900">{{ roles.length }}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="lg:col-span-2 bg-white rounded-xl sm:rounded-2xl border border-gray-200 overflow-hidden shadow-sm order-3 lg:order-2">
                 <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                    <h3 class="text-sm sm:text-base font-semibold text-gray-900">Organization Details</h3>
                 </div>
                 <div class="p-4 sm:p-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-4 sm:gap-y-6 gap-x-4">
                       <div v-if="org.primary_operating_region">
                          ...
                          <dd class="text-sm font-semibold text-gray-900 flex items-center gap-2">
                             <span class="w-2 h-2 rounded-full bg-indigo-500 flex-shrink-0"></span> <span class="truncate">{{ org.primary_operating_region }}</span>
                          </dd>
                       </div>
                       ...
                       <div class="sm:col-span-2 pt-4 mt-2 border-t border-gray-100" v-if="org.slug">
                          <dt class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-2">Public Identifier (Slug)</dt>
                          <dd class="flex items-center gap-2">
                             <code class="bg-gray-100 text-gray-600 px-2 py-1 rounded text-xs sm:text-sm border border-gray-200 break-all">{{ org.slug }}</code>
                          </dd>
                       </div>
                    </div>
                 </div>
            ...
            <div v-else-if="activeTab === 'members'" class="space-y-4">
              ...
              <!-- Desktop Table View -->
              <div class="hidden md:block bg-white rounded-xl sm:rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
                <table class="w-full">
                  ...
                  <tbody class="divide-y divide-gray-100">
                    ...
                    <tr v-for="member in members" :key="member.id" class="hover:bg-gray-50/80 transition-colors">
                      <td class="px-6 py-4">
