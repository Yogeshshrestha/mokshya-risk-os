...
// Explicitly mark as client-only page (OAuth callbacks must run on client)
definePageMeta({
  layout: false
})
...
onMounted(async () => {
  try {
    ...
    const callbackUrl = storedRedirectUri || redirectUri || `${window.location.origin}/google/callback`
